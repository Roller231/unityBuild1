...
export class CrashEngine {
	public static CrashSpeed = 0.00012;
    ...
    public getElapsedPayout(elapsedTime: number) {
        ...
        // при очень малых значениях рост заметнее
    ...
    public getElapsedPosition(elapsedTime: number) {
        // добавим мягкую кривизну в центре
        const elapsedPayout = this.getElapsedPayout(elapsedTime) - 1;
        const x = elapsedTime * this.xIncrement;
        const y = this.plotHeight - Math.pow(elapsedPayout, 0.9) * this.yIncrement;
        return { x, y };
      }
